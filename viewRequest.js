...
function onApproveRequest(requestId){
    let approveRequestElement = document.getElementById(requestId);
    requestContainer.removeChild(approveRequestElement);
    let approveElementIndex = RequestList.findIndex(function(eachRequest){
        let eachRequestId = "request" + eachRequest.uniqueNo; 
        if (eachRequestId===requestId){
            return true;
        }else{
    ...
    let approvedList = getApprovedListFromLocalStorage();
    console.log(RequestList[approveElementIndex]);
    approvedList.push(RequestList[approveElementIndex]);
    RequestList.splice(approveElementIndex,1);
    localStorage.setItem("RequestList", JSON.stringify(RequestList));
    localStorage.setItem("approveList", JSON.stringify(approvedList));
}    
